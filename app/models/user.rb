class User < ActiveRecord::Base
  ...
  ## want	item	itemをwantする。
  def want(item)
    wants.find_or_create_by(item_id: item.id)
  end
  

  ## want?	item	itemをwantしている場合true、wantしていない場合falseを返す。
  def want?(item)
    want_items.include?(item)
  end

  ## unwant	item	itemのwantを解除する。
  def unwant(item)
    want = wants.find_by(item_id: item.id)
    want.destroy if want 
  end

  ## have	item	itemをhaveする。
  def have(item)
    haves.find_or_create_by(item_id: item.id)
  end

  ## have?	item	itemをhaveしている場合true、haveしていない場合falseを返す。
  def have?(item)
    have_items.include?(item)
  end

  ## unhave	item	itemのhaveを解除する
  def unhave(item)
